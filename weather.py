...
def process_argument():
    ...
    subparsers = parser.add_subparsers(
        dest="cmd", metavar="cmd", required=True
    )

    from_config = subparsers.add_parser("config", help="使用設定檔匯入設定")
    from_config.add_argument(
        "config_file", type=argparse.FileType("r"), help="設定檔位置"
    )

    from_args = subparsers.add_parser("exec", help="從命令列輸入設定")
    from_args.add_argument(
        "-u", "--username", type=str, help="登入帳號", default=None
    )
    from_args.add_argument(
        "-p", "--password", type=str, help="登入密碼", default=None
    )
    from_args.add_argument(
        "-k", "--apikey", type=str, help="中央氣象局授權碼", default=None
    )
    from_args.add_argument(
        "-b", "--board", type=str, help="發文看板", default=None
    )
    from_args.add_argument(
        "-c",
        "--host",
...
def CWB_data(dataid, apikey):
    return requests.get(
        ...
        + f"?Authorization={apikey}&format=JSON"
...
def datetime2str(dt):
    return (
        ...
        + f"{dt.hour:02d}時{dt.minute:02d}分"
    )
...
def generate_post_content(data):
    issue_time = datetime.strptime(
        data["datasetInfo"]["issueTime"], "%Y-%m-%dT%H:%M:%S%z"
    )
...
"""
    for pos in data["location"]:
        ...
        content += (
            "＊{name}　{descript}{rain}％　　{min_t} - {max_t}\n".format(
                name=pos["locationName"],
                descript=weather_element["Wx"]["parameter"][
                    "parameterName"
                ].ljust(15, "　"),
                max_t=tcolor(
                    weather_element["MaxT"]["parameter"][
                        "parameterName"
                    ].rjust(2, " ")
                ),
                min_t=tcolor(
                    weather_element["MinT"]["parameter"][
                        "parameterName"
                    ].rjust(2, " ")
                ),
                rain=rcolor(
                    weather_element["PoP"]["parameter"]["parameterName"].rjust(
                        3, " "
                    )
                ),
            )
